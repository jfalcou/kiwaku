              ...
              )
{
  ...
  b.run_function("std::find_if", fct_std);
  std::cout << " OK!\n";
...
{
  const std::size_t d0 = 1024 * 1024 * 16; // 256
  auto convert_func = [=](auto item)
  {
    return
    static_cast<int>(std::cos(std::sin(std::cos(std::sin(static_cast<float>(item * 11) * 7.) * 3.8) * 8.7) * 9.48) * 89647681.) % 1784586545;
  };
  find_test("find-if compute-bound", "find-if_compute-bound_2025-06-05_01h45_last-pos.txt", convert_func, d0, d0-2);
  std::cout << "\n\n";
};
...
{
  const std::size_t d0 = 1024 * 1024 * 300;
  auto convert_func = [=](auto item) { return item; };
  find_test("find-if memory-bound", "find-if_memory-bound_2025-06-05_01h45_last-pos.txt", convert_func, d0, d0-2);
  std::cout << "\n\n";
};
...
TTS_CASE("Benchmark - find-if, compute-bound, middle")
{
  ...
  find_test("find-if compute-bound", "find-if_compute-bound_2025-06-05_01h45_middle.txt", convert_func, d0, d0 / 2);
  std::cout << "\n\n";
};
...
TTS_CASE("Benchmark - find-if, memory-bound, middle")
{
  const std::size_t d0 = 1024 * 1024 * 300;
  auto convert_func = [=](auto item) { return item; };
  find_test("find-if memory-bound", "find-if_memory-bound_2025-06-05_01h45_middle.txt", convert_func, d0, d0 / 2);
  std::cout << "\n\n";
};
