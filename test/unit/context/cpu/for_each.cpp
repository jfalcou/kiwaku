...
TTS_CASE("Check for kwk::for_each(kwk::cpu, func, shape)")
{
  auto shp = kwk::of_size(4,kwk::fixed<6>,3ULL);

  std::int32_t count = 0;
  kwk::for_each(kwk::cpu, [&](auto...) { ++count; }, shp);
...
TTS_CASE("Check for kwk::for_each(kwk::cpu, func, container) 1D")
{
  ...
  kwk::for_each(kwk::cpu, [&](auto e) { ++count; total += e; }, view);
...
TTS_CASE("Check for kwk::for_each(kwk::cpu, func, container) 1D with float")
{
...
TTS_CASE("Check for kwk::for_each(kwk::cpu, func, container) 1D with std::uint64_t")
{
...
TTS_CASE("Check for kwk::for_each(kwk::cpu, func, container) 1D size 0")
{
  using data_type = int;
...
TTS_CASE("Check for kwk::for_each(kwk::cpu, func, container) 2D")
{
  using data_type = int;
...
TTS_CASE("Check for kwk::for_each(kwk::cpu, func, container) 3D with float")
{
  using data_type = float;
  const std::size_t d0 = 20;
  const std::size_t d1 = 41;
...
TTS_CASE("Check for kwk::for_each(kwk::cpu, func, container) 4D")
{
  using data_type = int;
  const std::size_t d0 = 20;
  const std::size_t d1 = 41;
...
TTS_CASE("Check for kwk::for_each_index(kwk::cpu, func, container) 1D")
{
  using data_type = int;
  const std::size_t input_size = 20;
  std::array<data_type, input_size> input;
  ...
  kwk::for_each_index(kwk::cpu, [&](auto& e, auto i0) { ++count; total += e; e = i0 + 1; }, view);
...
TTS_CASE("Check for kwk::for_each_index(kwk::cpu, func, container) 1D with float")
{
  using data_type = float;
  const std::size_t input_size = 20;
  std::array<data_type, input_size> input;
...
TTS_CASE("Check for kwk::for_each_index(kwk::cpu, func, container) 1D with std::uint64_t")
{
  using data_type =  std::uint64_t;
  const std::size_t input_size = 20;
  std::array<data_type, input_size> input;
...
TTS_CASE("Check for kwk::for_each_index(kwk::cpu, func, container) 1D size 0")
{
  using data_type = int;
  std::vector<data_type> input;
  input.resize(0);
...
TTS_CASE("Check for kwk::for_each_index(kwk::cpu, func, container) 2D")
{
  using data_type = int;
  ...
  kwk::for_each_index(kwk::cpu, [&](auto& e, auto i0, auto i1) { ++count; total += e; e = pos_to_value(i0, i1); }, view);
...
TTS_CASE("Check for kwk::for_each_index(kwk::cpu, func, container) 3D with float")
{
  using data_type = float;
...
TTS_CASE("Check for kwk::for_each_index(kwk::cpu, func, container) 4D")
{
  using data_type = int;
  const std::size_t d0 = 2;
  const std::size_t d1 = 10;
