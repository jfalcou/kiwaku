...
TTS_CASE("Check for kwk::reduce(kwk::simd, in) 1D")
{
  using data_type = int;
...
TTS_CASE("Check for kwk::reduce(kwk::simd, in, func) and kwk::reduce(kwk::simd, in, func, init) 1D")
{
  using data_type = int;
  const std::size_t d0 = 47100;
  const std::size_t input_size = d0;
  ...
  auto res = kwk::reduce(kwk::simd, view_in, monoid, 0);
