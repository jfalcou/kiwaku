                    ...
                    )
{
  ...
  std::size_t total_data_size = total_number_of_elements * sizeof(DATA_TYPE) * 3;

  // std::cout << "L2_size(" << L2_size << ") repeat_count(" << repeat_count << ")\n";

  // The algorithm writes into out and reads from in.
  std::vector<DATA_TYPE> vector_in(L2_size);
  std::vector<DATA_TYPE> vector_out(L2_size);
  ...
  reset_data(vector_in);

  // Kiwaku views
  auto kwk_in   = kwk::view{kwk::source = vector_in.data()  , kwk::of_size(L2_size)};
  auto kwk_out  = kwk::view{kwk::source = vector_out.data() , kwk::of_size(L2_size)};
  ...
  auto bench_kiwaku = [&](auto&& context, std::string context_name, auto fct)
  {
    ...
    auto fct_generic = [&]()
    {
      for (std::size_t repeat_co = 0; repeat_co < repeat_compute_intensity; ++repeat_co)
      {
        kwk::transform(context, fct, kwk_out, kwk_in);
      }
  ...
  auto fct_hand = [&]()
  {
    for (std::size_t repeat_co = 0; repeat_co < repeat_compute_intensity; ++repeat_co)
    {
      for (std::size_t i = 0; i < L2_size; ++i)
      {
        vector_out[i] = func_generic(vector_in[i]);
      }
    }
    return vector_out[L2_size / 2];
  };
  ...
  std::copy(vector_out.begin(), vector_out.end(), truth_out.begin());


  // ====== Kiwaku CPU context ======
  bench_kiwaku(::kwk::cpu, "kwk CPU", func_generic);
  if constexpr(enable_check) TTS_ALL_RELATIVE_EQUAL(vector_out, truth_out, 1);
      ...
      {
        {
          ...
          auto proxy_out = ctx_gpu.out(kwk_out);
          auto proxy_in  = ctx_gpu.in(kwk_in);

          auto fct_transforms = [&]()
          {
            for (std::size_t repeat_co = 0; repeat_co < repeat_compute_intensity; ++repeat_co)
            {
              kwk::transform_proxy(ctx_gpu, func_generic, proxy_out, proxy_in);
            }
            return kwk_out(L2_size / 2) + kwk_out(L2_size / 2);
          };
...
TTS_CASE("Benchmark - transform, compute-bound")
{
  ...
  std::size_t compute_intensity = 1;


  sutils::printer_t::head("Benchmark - transform, compute-bound (L2 " + l2_str + ")", true);

  transform_test<DATA_TYPE>( "transform trigo"
...
TTS_CASE("Benchmark - transform, memory-bound")
{
  ...
  std::string hname = sutils::get_host_name();
  // Total data to process
        if (hname == "parsys-legend")          { total_size = 2 * gio * kwk::bench::LEGEND_LOAD_FACTOR; L2_size = total_size; } 
        // if (hname == "parsys-legend")          { L2_size = 512 * kio; total_size = 1 * gio * kwk::bench::LEGEND_LOAD_FACTOR; } 
