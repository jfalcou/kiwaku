                  ...
                  )
{
  ...
  auto generic_bench = [&](auto&& context, DATA_TYPE& return_) -> DATA_TYPE
  {
    ...
    b.run_function("Kiwaku SIMD " + kwk::bench::EVE_COMPILER_FLAG, fct_kwk_sycl_generic, [&]{ reset_data(data); });
    return return_;
  };
...
TTS_CASE("Benchmark - for_each, compute-bound_trigo")
{
  if (::kwk::bench::enable_global)
  {
    ...
    std::string hname = sutils::get_host_name();
         if (hname == "parsys-legend")          { size =   256 * mio * kwk::bench::LEGEND_LOAD_FACTOR; } 
    ...
    for_each_test<DATA_TYPE>("for_each compute-bound_trigo", "for_each_compute-bound_trigo_" + kwk::bench::EVE_COMPILER_FLAG + ".bench", map_func, size);
    std::cout << "\n\n";
  }
  else
  {
...
TTS_CASE("Benchmark - for_each, memory-bound")
{
  if (::kwk::bench::enable_global)
  {
    ...
    for_each_test<DATA_TYPE>("for_each memory-bound", "for_each_memory-bound_" + kwk::bench::EVE_COMPILER_FLAG + ".bench", map_func, size);
    std::cout << "\n\n";
  }
  else
  {
