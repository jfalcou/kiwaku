...
struct box
{
  ...
  box(std::size_t n, A a)
    : alloc(std::move(a)), data_(reinterpret_cast<float*>(allocate(alloc, n * sizeof(float)))), size_(n)
  {
  }

  box(box const& that) : box(that.size_, that.alloc) { std::copy(that.begin(), that.end(), data()); }
  ...
  ~box()
  {
    if (size_ != 0) deallocate(alloc, data_);
  }
