...
if display_speedup:
  ...
  # else:
  cpu_list   = pu.make_absolute_list(l_context_cpu      , "med_copy_and_kernel")
  sycl_cpu   = pu.make_absolute_list(l_context_sycl_cpu , "med_copy_and_kernel")
  sycl_gpu   = pu.make_absolute_list(l_context_sycl_gpu , "med_copy_and_kernel")
  ...
  if display_context_cpu:
    plt.plot  (range(1, len(cpu_list)+1), cpu_list, color="grey", label="context::cpu - " + cpu_name , linestyle="solid"  , linewidth=line_width)
