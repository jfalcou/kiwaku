...
{
  kwk::array<float,kwk::extent[9],kwk::stack_> a;
...
{
  kwk::array<float,kwk::extent[9],kwk::stack_> ref;

  float k = 1.f;
  for(auto& e : ref) e = k++;

  kwk::array<float,kwk::extent[9],kwk::stack_> a = ref;
  TTS_EXPECT( std::equal(ref.begin(),ref.end(),a.begin()) );
}

TTS_CASE( "Automatic storage, constexpr 1D shape array - Move constructor" )
{
  kwk::array<float,kwk::extent[9],kwk::stack_> ref, ref2;

  float k = 1.f;
  for(auto& e : ref) e = k++;
  ref2 = ref;

  kwk::array<float,kwk::extent[9],kwk::stack_> a = std::move(ref2);
  TTS_EXPECT( std::equal(ref.begin(),ref.end(),a.begin()) );
}
...
TTS_CASE( "Automatic storage, constexpr 1D shape array - Move constructor" )
{
  kwk::array<float,kwk::extent[9],kwk::stack_> a, ref, ref2;
