...
namespace kwk::__
{
  template<typename... Coords> constexpr auto setup_linear_interpolation(Coords... p) noexcept
  {
    return kumi::fold_left(
      [](auto acc, auto v) {
        ...
        auto cmp = kumi::map([&](auto e) { return kumi::push_back(e, mp); }, coord);
        auto cnp = kumi::map([&](auto e) { return kumi::push_back(e, mp + 1); }, coord);

        return kumi::make_tuple(kumi::cat(cmp, cnp), kumi::cat(cf0, cf1));
      },
      kumi::make_tuple(p...), kumi::make_tuple(kumi::make_tuple(kumi::tuple{}), kumi::tuple{1.f}));
  }

  struct linear_
  {
    template<typename... Coords> auto interpolate(auto const& data, Coords... p) const
    {
      ...
      return kumi::inner_product(kumi::pop_front(pos), kumi::pop_front(coeff), kumi::front(pos) * kumi::front(coeff));
    }
  };

  struct floor_
  {
    template<typename Container, typename... Coords> auto interpolate(Container const& data, Coords... p) const
    {
