...
namespace kwk
{
  ...
  KWK_FORCEINLINE constexpr auto pick(detail::view_, detail::type_, Settings const& opts)
  {
    ...
    {
      return as<typename source_traits<decltype(pick(source,opts))>::base_type>;
    }
    else
    {
      return opts[type];
  ...
  template<rbr::concepts::settings Settings>
  KWK_FORCEINLINE constexpr auto pick(detail::table_, detail::type_, Settings const& opts)
  {
