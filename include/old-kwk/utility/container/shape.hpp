...
namespace kwk
{
  ...
  template<std::int32_t N, auto... D> constexpr auto compress(shape<D...> const&) noexcept;
  ...
  //====================================================================================================================
  template<auto... D> struct shape : __::prefilled_t<D...>::type
  {
    ...
    template<auto... D2> KWK_PURE friend constexpr bool operator==(shape const& a, shape<D2...> const& b) noexcept
    {
      constexpr auto sz = std::min(static_order, shape<D2...>::static_order);
      bool result = true;
      kumi::for_each([&](auto aa, auto bb) { result = result && (aa == bb); }, compress<sz>(a), compress<sz>(b));
      return result;
    }
    ...
    //==============================================================================================
    template<auto... D2> KWK_PURE constexpr bool fit_constraints(shape<D2...> const& ref) const noexcept
    {
    ...
    requires(sizeof...(Slicers) <= static_order)
    {
      ...
      auto sliced = kumi::map_index([&](auto i, auto m) { return reshape(shd, m, i); }, kumi::tie(s...));
  ...
  template<concepts::extent... T> shape(T...) -> shape<to_descriptor(T{})...>;

  //====================================================================================================================
  //! @brief Generates a kwk::shape from a list of @ref glossary-extent
  //! @param  d        Variadic pack of @ref glossary-extent
  //====================================================================================================================
  template<int..., concepts::extent... D> KWK_TRIVIAL constexpr auto of_size(D... d)
  {
  ...
  //! @param  d         Variadic pack of @ref glossary-extent
  //====================================================================================================================
  template<std::integral SizeType, int..., concepts::extent... D> KWK_TRIVIAL constexpr auto of_size(D... d)
  {
  ...
  //====================================================================================================================
  template<int..., kumi::concepts::product_type T> KWK_TRIVIAL constexpr auto of_size(T const& t)
  {
  ...
  //! @param  t         Tuple of  @ref glossary-extent
  //====================================================================================================================
  template<typename SizeType, int..., kumi::concepts::product_type T> KWK_TRIVIAL constexpr auto of_size(T const& t)
  {
...
struct std::tuple_element<N, kwk::shape<D...>> : std::tuple_element<N, typename kwk::shape<D...>::parent>
{
};
#endif
