...
namespace kwk
{
  template<concepts::extent... D> constexpr auto with_strides(D... d) noexcept;
  ...
  //================================================================================================
  template<auto... Strides> struct stride : __::prefilled_t<Strides...>::type
  {
    ...
    static constexpr auto is_unit = []() {
      ...
      if constexpr (concepts::static_axis<decltype(d0)>) return d0.value == 1;
      else return false;
    }();
    ...
    {
      auto sliced = kumi::map_index([&](auto i, auto m) { return restride(*this, m, i); }, kumi::tie(s...));
  ...
  template<concepts::extent... T> stride(T...) -> stride<to_descriptor(T{})...>;
  ...
  template<kumi::concepts::product_type D> constexpr auto with_strides(D d) noexcept
  {
  ...
  template<auto... D> KWK_PURE constexpr auto as_stride(shape<D...> s) noexcept
  {
  ...
  template<std::int32_t N, auto... D> constexpr auto compress(stride<D...> const& s) noexcept
  {
...
struct std::tuple_element<N, kwk::stride<D...>> : std::tuple_element<N, typename kwk::stride<D...>::parent>
{
};
#endif
