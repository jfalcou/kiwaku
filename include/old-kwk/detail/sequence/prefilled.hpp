...
namespace kwk::__
{
  ...
  template<auto... D> struct prefilled_base
  {
    ...
    {
      if constexpr (dynamic_size == 0) { return empty_tuple{}; }
      else
      {
  ...
  //====================================================================================================================
  template<auto... D> struct prefilled : prefilled_base<D...>::type
  {
    ...
    constexpr void __construct(auto def, concepts::numeric_extent auto... vs) noexcept
    {
      ...
      kumi::for_each_index(
        [&]<typename I>(I, auto v) {
          if constexpr (contains(I::value)) { __get<I::value>() = kwk::as_dimension(v, def); }
          else
          {
            KIWAKU_ASSERT((v == _) || v == __get<I::value>(), "[KWK] - Runtime/Compile-time mismatch for axis "
                                                                << kumi::get<I::value>(descriptors) << " as " << v
      ...
      if constexpr (!is_fully_static)
      {
        ...
        kumi::for_each(
          [&]<typename X>(X x) {
            ...
            if constexpr (contains(idx)) { (*this)[x] = kwk::as_dimension(input[x.base()], def); }
            else
            {
    ...
    template<concepts::extent... V> static constexpr auto __is_ordered()
    {
      ...
      kumi::for_each_index(
        [&]<typename V>(auto i, V value) {
          if constexpr (contains(i)) get<i>(*this) = value;
            ...
            KIWAKU_ASSERT(value == get<i>(*this), "[KWK] - Runtime/Compile-time mismatch at index "
                                                    << i << " - Expected " << get<i>(*this) << " but " << value
                                                    << " was provided");
        },
        other);
    }
    ...
    // Static axis access
    template<std::size_t N> KWK_TRIVIAL constexpr auto axis() const noexcept
    {
      auto&& d = get<N>(descriptors);
      return d.base();
    }

    // Static tuple access impl
    template<std::size_t N> KWK_TRIVIAL constexpr decltype(auto) __get() const& noexcept
    {
    ...
    template<std::size_t N> KWK_TRIVIAL constexpr decltype(auto) __get() & noexcept
    {
      if constexpr (contains(N)) return storage_type::operator[](kumi::index<setup.index[N]>);
      else return fixed<get<N>(descriptors).value>;
    }
    ...
    requires(is_dynamic && static_size > 0)
    {
      ...
      KIWAKU_ASSERT(contains(idx), "[KWK] - Access at index " << idx << " overwrites a compile-time value of "
                                                              << kumi::to_tuple(*this));
    ...
    template<concepts::axis A> static constexpr auto __find_axis(A)
    {
    ...
    template<auto... Os> constexpr bool is_equivalent(prefilled<Os...> const&) const noexcept
    {
    ...
    template<auto... Os> constexpr bool is_compatible(prefilled<Os...> const& other) const noexcept
    {
      ...
      else
      {
        return kumi::fold_left(
          ...
          kumi::zip(descriptors, other.descriptors), true);
      }
    }

    template<auto... Os> constexpr bool is_similar(prefilled<Os...> const& other) const noexcept
    {
      if constexpr (static_size != prefilled<Os...>::static_size) return false;
      else
      {
  ...
  //====================================================================================================================
  template<auto... D> struct prefilled_t
  {
...
//======================================================================================================================
template<auto... Desc>
struct std::tuple_size<kwk::__::prefilled<Desc...>> : std::integral_constant<std::int32_t, sizeof...(Desc)>
{
};

template<std::size_t N, auto... Desc> struct std::tuple_element<N, kwk::__::prefilled<Desc...>>
{
